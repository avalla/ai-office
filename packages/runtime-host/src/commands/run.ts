  ...
  if (command === "run:tick") {
    ...
    for (const run of queued) {
      ...
      if (gateway !== undefined) {
        ...
        if (missing.length > 0)
          throw new CliUsageError(
            `Run ${snapshot.id} is assigned ${selection.modelRef}, but the Runtime host has no usable ${missing.join(", ")}. A managed Runtime reads provider credentials only from the credentials directory in AI_OFFICE_HOME (ai-office credential set); a foreground Runtime reads its environment first. Credentials are loaded at Runtime start; see model:check. No runs were started.`,
          );
        continue;
      }
