...
For inspection, run `ai-office status <path> --json`. Distinguish local repository identity, Runtime association, authoritative project availability, office state, onboarding state, and each client integration. A valid local identity with an unreachable Runtime host is not the same as an uninstalled project. Use `--offline` only when intentionally inspecting repository-local state without contacting the Runtime; it reports host state as `not_checked` and health as `unverified` when local inspection finds no problems; observed integration issues or an invalid binding instead produce `needs_attention` and exit 1. Host state `not_checked` is not evidence that the Runtime is unavailable, so never recommend starting it on that basis. Relative paths mean the directory the command runs in, never the Runtime host's.
