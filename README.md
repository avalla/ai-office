...
A file-based virtual agency system for AI-assisted software development. AI Office now ships as a host-neutral core plus thin adapters for Codex, Windsurf, Claude Code, or a base markdown+CLI workflow, with generated adapter wrappers built from a shared neutral manifest.
...
### Scope Today
- The generated layer currently covers the core operational wrapper set: `office`, `office-route`, `office-advance`, `office-doctor`, `office-milestone`, `office-setup`, `office-status`, `office-task-create`, `office-task-integrate`, `office-task-move`, `office-task-update`, and `office-validate`
- Some legacy wrapper files still remain hand-maintained and will be migrated in later passes
...
The adapter wrapper layer is generated from a neutral manifest, so host-specific files stay aligned over time.

At the moment this generation model covers the core operational wrappers first; the remaining legacy wrappers are still being migrated.
...
```bash
$office-_meta
# Output: Framework v1.8.0 installed, v1.9.0 available. Run ./update.sh
