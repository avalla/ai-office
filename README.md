...
A file-based virtual agency system for AI-assisted software development. AI Office now ships as a host-neutral core plus thin adapters for Codex, Windsurf, Claude Code, or a base markdown+CLI workflow, with a single adapter source of truth in the neutral manifest plus shared command templates.
...
- Adapter-specific wrapper files such as `AGENTS.md` + `.codex/skills/`, `AGENTS.md` + `.windsurf/rules/` + `.windsurf/workflows/`, or `CLAUDE.md` + `.claude/skills/`
...
This rebuilds the generated adapter wrappers from the neutral sources in `src/adapter-manifest.ts` and `skeleton/core/templates/`.

This step is mainly for framework maintainers. Projects consuming AI Office do not need to run it during normal usage.
...
Examples below use Codex shortcut syntax. Windsurf and Claude Code use slash workflows through their adapters, and the base adapter can call the `ai-office` CLI directly.
...
### New Build Model
- **Neutral manifest**: the core command wrappers now have a shared source of truth in `src/adapter-manifest.ts`
- **Generated adapter files**: Codex skills, Claude Code skills, Windsurf workflows, and adapter instruction files are now generated by `bun run build:adapters`
- **Shared instruction template**: adapter-level rule files are rendered from `skeleton/core/templates/adapter-instructions.md.tmpl`
- **Shared command templates**: long-form Codex and Claude Code skills now render from `skeleton/core/templates/skills/`
- **Generated shell metadata**: `install.sh` and `update.sh` now consume adapter metadata generated from the same manifest

### Why It Matters
- Reduces drift across adapters
- Makes new adapters easier to add
- Keeps host-specific wrappers thin while the workflow semantics stay neutral

### Scope Today
- The generated layer covers the core operational wrapper set across Codex, Claude Code, and Windsurf
- Codex and Claude Code also render the longer-form command wrappers from shared neutral templates
- Windsurf intentionally keeps the slimmer workflow surface around the core operational commands
...
generated/adapter-metadata.sh              ← Derived shell metadata for install/update
```
...
- **One supported host adapter**: Codex, Windsurf, Claude Code, or the base markdown+CLI adapter
...
```bash
$office-_meta
# Output: Framework v1.8.0 installed, v1.9.1 available. Run ./update.sh

# Or manually
./update.sh [project-root] [--adapter=codex|windsurf|claude-code|base]
```
