...
A file-based virtual agency system for AI-assisted software development. AI Office now ships as a host-neutral core plus thin adapters for Codex, Windsurf, Claude Code, OpenCode, or a base markdown+CLI workflow, with a single adapter source of truth in the neutral manifest plus shared command templates.
...
- **Generated adapter files**: Codex skills, Claude Code skills, OpenCode commands, Windsurf workflows, and adapter instruction files are now generated by `bun run build:adapters`
...
- **Shared command templates**: long-form Codex skills, Claude Code skills, and OpenCode commands now render from `skeleton/core/templates/skills/`
- **Generated shell metadata**: `install.sh` and `update.sh` now consume adapter metadata generated from the same manifest
- **Legacy cleanup on update**: `update.sh --prune-legacy` can remove stale AI Office artifacts from previous adapters while preserving the active one
- **New OpenCode adapter**: installable via `--adapter=opencode` with `opencode.json` plus `.opencode/commands/`
...
- Keeps OpenCode compatible with local or custom providers while the workflow layer stays unchanged

### Scope Today
- The generated layer covers the core operational wrapper set across Codex, Claude Code, OpenCode, and Windsurf
- Codex, Claude Code, and OpenCode also render the longer-form command wrappers from shared neutral templates
...
```bash
$office-_meta
# Output: Framework v1.10.0 installed, v1.11.0 available. Run ./update.sh
