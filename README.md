...
A file-based virtual agency system for AI-assisted software development. Orchestrate 22 specialized agents through a structured pipeline with milestones, kanban task board, role-based guidance, code quality rules, and loop guards—all as Codex skills.
...
- `AGENTS.md` — base quality rules (auto-loaded by Codex)
- `.codex/skills/` — 23 AI Office skills
...
Use the CLI for state mutations and let the Codex skills orchestrate higher-level workflow around it.
...
- [Base Rules (AGENTS.md)](#-base-rules-agentsmd)
- [Updating](#updating)
...
graph TD
    ...
    AGENTS["AGENTS.md<br/>(base rules)"]
    ...
    CODEX --> SKILLS["skills/<br/>(23 skills)"]
...
```
AGENTS.md                        ← Base quality rules (always active)

.codex/
└── skills/                      ← 23 AI Office skills
...
│   └── qa-checklist.md
│
├── addons/                      ← Opt-in rules (uncomment in AGENTS.md)
...
`install.sh` places `AGENTS.md` at the project root — Codex loads this automatically every session. It encodes:
...
To activate project-specific rules, copy the addon contents you need into `AGENTS.md`:
...
- **[Codex](https://claude.ai/code)** — IDE integration and CLI
