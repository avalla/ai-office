...
Description: AI Office command: office-meta
...
1. Read `.opencode/.version` (the installed version stamp). If it doesn't exist, report "unknown".
...
Wrapper files installed: .opencode/commands/ (24 expected files)
```
...
5. Also check: are all 24 expected wrapper files present in `.opencode/commands/`? List any missing ones.
