...
1. Read `.claude/skills/.version` (the installed version stamp). If it doesn't exist, report "unknown".
...
Skills installed: .claude/skills/ (16 files)
```
...
5. Also check: are all 23 expected skills present in `.claude/skills/`? List any missing ones.
