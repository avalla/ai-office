 ...
 */
function fakeVendorGateway(runtimeHome: string) {
  const requests: {
...
test("a managed Runtime executes a gateway run with its Runtime home credential and leaks it nowhere", async () => {
  ...
  try {
    ...
    // What a generated service definition sets, plus an ambient key a service
    // manager environment might still carry.
