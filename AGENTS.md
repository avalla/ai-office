...
- Stateful product commands go through the authoritative Runtime, currently hosted by the local daemon. The CLI is a Runtime client over local IPC; it never falls back to an embedded writer. Local help and product version, explicit `status --offline`, compatible degraded read-only status, source-linked `update`, and `runtime:purge` are the narrow offline paths. Update performs only health presence checks for the selected user and distribution development Runtime homes before local program maintenance. Purge refuses to run while the Runtime host is reachable.
